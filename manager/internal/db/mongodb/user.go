...
func (m *mongoDB) UpdateUser(ctx context.Context, id string, user *pb.User) (*pb.User, error) {
	...
	// 备份 openid 和unid
	baseuser := &pb.User{}
	_ = m.CUser().FindOne(ctx, filter).Decode(baseuser)
	user.Openid = baseuser.Openid
	user.Unionid = baseuser.Unionid

	update := bson.M{"$set": user}
	err := m.CUser().FindOneAndUpdate(ctx, filter, update, options.FindOneAndUpdate().SetReturnDocument(options.After)).Decode(newData)
	if err != nil {
		return newData, err
	}
	return newData, nil
...
func (m *mongoDB) ChangeAuthorization(ctx context.Context, username, password, openid string) (err error) {
	// unique value check
	if openid == "" {
		openid = username
	}
	filter := bson.M{"username": openid}
