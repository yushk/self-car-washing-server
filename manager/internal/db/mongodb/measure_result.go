...
func (m *mongoDB) UpdateMeasureResult(ctx context.Context, id string, data *pb.MeasureResult) (*pb.MeasureResult, error) {
	newData := &pb.MeasureResult{}
	filter := bson.M{"id": id}
	data.Id = id
	update := bson.M{"$set": data}
	err := m.CMeasureResult().FindOneAndUpdate(ctx, filter, update, options.FindOneAndUpdate().SetReturnDocument(options.After)).Decode(newData)
	if err != nil {
		return newData, err
	}
	return newData, nil
