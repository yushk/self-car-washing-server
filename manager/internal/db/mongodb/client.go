...
import (
	...
	config2 "github.com/yushk/sport_backend/pkg/config"
...
const (
	...
	collectionCourse           = "course"            // 课程表
	collectionWork             = "work"              // 作业表
	collectionWorkSubmit       = "work_submit"       // 作业提交表
	collectionComment          = "comment"           // 评论信息表
	collectionSportItem        = "sport_item"        // 运动项目表
	collectionHomeWork         = "home_work"         // 作业表
	collectionSolution         = "solution"          // 学生提交作业表
	collectionEvaluateInfo     = "evaluate_info"     // 评价信息表
	collectionSportType        = "sport_type"        // 运动类别表
)
...
// CMovePrescription 运动处方表集合句柄
func (m *mongoDB) CMovePrescription() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionMovePrescription)
}

// CCourse 课程表集合句柄
func (m *mongoDB) CCourse() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionCourse)
}
...
// CComment 评论信息表集合句柄
func (m *mongoDB) CComment() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionComment)
}

// CSportItem 运动项目表集合句柄
func (m *mongoDB) CSportItem() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionSportItem)
}

// CHomeWork 作业表集合句柄
func (m *mongoDB) CHomeWork() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionHomeWork)
}

// CSolution 学生提交作业表集合句柄
func (m *mongoDB) CSolution() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionSolution)
}

// CSolution 评价信息表集合句柄
func (m *mongoDB) CEvaluateInfo() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionEvaluateInfo)
}

// CSportType 运动类别表集合句柄
func (m *mongoDB) CSportType() *mongo.Collection {
	return m.client.Database(dbName).Collection(collectionSportType)
}
