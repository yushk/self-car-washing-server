...
func (s *Server) ChangePassword(ctx context.Context, req *pb.ChangePasswordRequest) (*pb.ChangePasswordReply, error) {
	err := s.db.ChangeAuthorization(ctx, req.Username, req.Password, req.Openid)
	if err != nil {
		return nil, status.Errorf(codes.Unauthenticated, req.Username+" change password failed")
	}
