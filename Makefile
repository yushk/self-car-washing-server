...
# Docker informations
# docker push docker.pkg.github.com/yushk/sport_backend
# DOCKER_REPO := https://hub.docker.com/explore/
DOCKER_REPO := registry.cn-beijing.aliyuncs.com/
DOCKER_PROJECTNAME := sport
DOCKER_NAMESPACE := yushk
DOCKER_TAG       := latest
...
.docker-build:
	docker build \
		...
		--label=$(DATE) \
		-t $(DOCKER_REPO)$(DOCKER_NAMESPACE)/${DOCKER_PROJECTNAME}_$(SERVICE_NAME):$(DOCKER_TAG) .
	docker push $(DOCKER_REPO)$(DOCKER_NAMESPACE)/${DOCKER_PROJECTNAME}_$(SERVICE_NAME):$(DOCKER_TAG)
	docker rmi $(DOCKER_REPO)$(DOCKER_NAMESPACE)/${DOCKER_PROJECTNAME}_$(SERVICE_NAME):$(DOCKER_TAG)
